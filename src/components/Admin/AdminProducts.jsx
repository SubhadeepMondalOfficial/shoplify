    ...
    <Layout>
      <div >
        <div className="flex justify-between items-center mb-6">
          ...
          <button className="bg-violet-600 text-white px-4 py-2 rounded flex justify-center items-center gap-2" onClick={()=>setProductModal(true)}>
            <IoIosAddCircle />
            New Product
          </button>
        </div>
          ...
          {products.map((item, index) => (
            <div key={index} className=" shadow-lg rounded-xl overflow-hidden">
              <div className="relative">
                ...
                />
                <input type="file" name="" id="" className="absolute left-0 top-0 w-full h-full opacity-0 cursor-pointer" onChange={(event)=>uploadProductImage(event, item)} />
              </div>
        ...
        {
          productModal && 
          <div ref={elementProductModal} className="animate__animated animate__fadeIn animate__faster absolute top-0 left-0 h-full w-full p-5 bg-black bg-opacity-80 flex justify-center items-center">
          <div className="animate__animated animate__zoomIn animate__faster w-6/12 p-4 rounded-lg relative bg-white">
            <button className="absolute top-3 right-3" onClick={handleModalClose}>
              <RxCross2 />
            </button>
            <h1 className="mb-4 font-semibold">New Product</h1>
            <form className="grid grid-cols-2 gap-4" onSubmit={createProduct}>
              <input
                type="text"
                name="title"
